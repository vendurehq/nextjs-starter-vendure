...
The checked-in template cannot contain its own Git commit hash, so initialization resolves the release tag once and records it.
...
Use a `.none.md` file only when a downstream-impacting diff genuinely has no downstream impact, and explain why. CI validates the note or exemption:

```bash
npm run upgrade:validate
```

Prepare a release without creating its tag:

```bash
npm run upgrade:release -- 1.1.0
```

The release command aggregates and consumes pending notes, updates the starter version, and generates `.upgrades/releases/v1.1.0/manifest.json` plus `guide.md`. Review and commit those artifacts before creating the immutable `v1.1.0` tag.
